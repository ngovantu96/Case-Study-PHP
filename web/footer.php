<footer>
		<div class="container">
			<div class="row">
				<div class="col-md-4">
					<div class="block clearfix">
                    <h4 class="connect-heading">TiMiStore.Shop</h4>
						<p>
						TiMiStore.Shop - Tự hào là thương hiệu thời trang Việt Nam mang lại những trải nghiệm mua sắm tốt nhất, giúp bạn thêm hứng khởi, tự tin và yêu bản thân hơn qua mỗi ngày khi được thể hiện cá tính của mình.
						</p>
					</div>
				</div> 
				<div class="col-md-4">
					<div class="block">
						<h4>Địa Chỉ</h4>
						<p ><i class="fa  fa-map-marker"></i> <span>TiMiStore.Shop</span>1000 Lý Thường Kiệt, Q1. TPHCM</p>
					</div>	<!-- End Of /.block -->
				</div> <!-- End Of Col-md-3 -->
				<div class="col-md-4">
					<div class="block">
						<h4>Kết Nối</h4>
						<!-- <div class="media"> -->
						<p> <i class="fa  fa-phone"></i> <span>Phone:</span> (+386) 40 123 456 </p>

						<p> <i class="fab fa-facebook mr-2"></i><span>FanPage TiMiStore</span></p>

						<p class="mail"><i class="fa fa-envelope mr-2"></i>Email: <span>info@sitename.com</span></p>
												
						
					</div>	<!-- End Of /.block -->
				</div> <!-- End Of Col-md-3 -->
			</div> <!-- End Of /.row -->
		</div> <!-- End Of /.Container -->
		


	<!-- FOOTER-BOTTOM Start
    ================================================== -->

		<!-- <div class="footer-bottom">
			<div class="container">
				<div class="row">
					<div class="col-md-12">
						
						<p class="copyright-text pull-right">Copy Right @2020 Designed by <a href="http://www.themexpert.com">Chú Bốn</a> All Rights Reserved</p>
					</div>	 End Of /.col-md-12 
				</div>	
			</div>	
		</div>>
    </footer>-->

  <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.16.0/umd/popper.min.js"></script>
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script>